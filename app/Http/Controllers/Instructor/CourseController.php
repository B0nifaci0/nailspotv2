...
class CourseController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware("can:Actualizar cursos")->only(
            "edit",
            "update",
            "goals"
        );
    ...
    public function create()
    {
        ...
        return view(
            ...
            compact("categories", "levels", "platforms")
        );
    }
    ...
    public function store(CourseRequest $request)
    {
        ...
        return redirect()
            ...
            ->with("info", "El curso se creo con exito!");
    }
    ...
    public function edit(Course $course)
    {
        ...
        return view(
            ...
            compact("course", "categories", "levels", "platforms")
        );
    }
    ...
    public function update(CourseRequest $request, Course $course)
    {
        ...
        if ($request->hasFile("image")) {
            $image = file_get_contents($request->file("image")->path());
            $base64Image = base64_encode($image);
            $course->image->url = $this->saveImages(
                $base64Image,
                "courses",
                $course->id
            );
        ...
        return redirect()
            ->route("instructor.courses.edit", $course)
            ->with("info", "El curso se actualizo con exito!");
    }
    ...
    public function studentTasks(Course $course, $id)
    {
        if ($course->students->contains("id", $id)) {
            $this->authorize("dicatated", $course);
            return view(
                "instructor.courses.student.tasks",
                compact("id", "course")
            );
        }
        return abort(404);
    ...
    public function status(Course $course)
    {
        if ($course->final == 0) {
            return redirect()
                ->route("instructor.courses.tasks", $course)
                ->with("error", "¡Por favor agrega las tareas del curso!");
        }
        if ($course->goals_count == 0 || $course->requirements_count == 0) {
            return redirect()
                ->route("instructor.courses.goals", $course)
                ->with(
                    "error",
                    "¡Por favor agrega las metas y requerimientos del curso!"
                );
        }
        if ($course->lessons_count == 0) {
            return redirect()
                ->route("instructor.courses.curriculum", $course)
                ->with("error", "¡Por favor agrega las lecciones del curso!");
        }
